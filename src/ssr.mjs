/******************************************************************
 * cb2p-avocats – SSR (Angular 18)              15 jun 2025 – OK
 ******************************************************************/
...
/* !! ligne manquante : charge le compilateur JIT ---------------- */
import '@angular/compiler';                // <<<<<<  ICI
...
app.use('/wp-json', async (req, res) => {
  try {
    const r = await fetch('https://cb2p-avocats.fr/wordpress/wp-json' + req.url);
...
app.get('*', async (req, res) => {
  try {
    ...
    res.status(200).send(tpl);            // client-side fallback
  }
});

/* ------------ 6. Lancement ------------------------------------ */
const PORT = process.env.PORT || 4000;
app.listen(PORT, () => console.log(`✅ SSR server ready on http://localhost:${PORT}`));
